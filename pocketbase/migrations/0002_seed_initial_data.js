migrate(
  (app) => {
    ...
    // Idempotência: verificar se já existe algum registro
    let existingConfig = null
    try {
      const records = app.findRecordsByFilter('configuracoes_site', '', '', 1, 0)
      if (records && records.length > 0) {
        existingConfig = records[0]
      }
    } catch (_) {}
    ...
    for (const card of seedCards) {
      try {
        app.findFirstRecordByData('cards_home', 'titulo', card.titulo)
